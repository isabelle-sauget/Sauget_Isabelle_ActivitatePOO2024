...
private:  // complex comercial has a cofetarie, magazin, masina
	Magazin magazin; //deja creat cu constructorul fara parametri!
	Cofetarie* cofetarie;
	int nrMasiniParcate;
...
public:
	...
	{
		this->magazin = mag;
		//se apeleaza operatorul egal din clasa magazin
		//ambele obiecte exista deja
		//this->cofetarie = cof; GRESIT, shallow copy
		...
		for (int i = 0; i < nrMasini; i++)
		{
			...
			//se apeleaza operatorul egal din 
			//clasa Masina, datorat relatiei has a
		}
	}



	ComplexComercial(ComplexComercial& cc)
	{
	...
	~ComplexComercial()
	{
		if (this->cofetarie)
		{
			...
			//un singur obiect de sters => fara []
		}
	...
	Magazin getMagazin() const //face pointerul this constant
	{
		return this->magazin;
	}
	...
	ComplexComercial operator--()
	{
		...
		{//am de unde sa sterg masini?
			this->nrMasiniParcate--;

			Masina* temp = new Masina[this->nrMasiniParcate];
			for (int i = 0; i < this->nrMasiniParcate; i++)
			{
				temp[i] = this->masiniParcate[i];
			}
			delete[]this->masiniParcate;
			this->masiniParcate = temp;
			//se apeleaza operatorul = pentru pointeri
		}
...
int main() {
	...
	cout << cc2;
	cc1 = cc2--;//returneaza starea initiala, adica cc2 simplu
	//cc1 = --cc2;
