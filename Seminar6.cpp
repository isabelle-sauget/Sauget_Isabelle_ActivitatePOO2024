...
public:
	...
	explicit operator int() //operatori cast
	{
		return this->nrAngajati;
	}
	...
	friend ostream& operator<<(ostream& output, Cofetarie c); //outline declarata

	friend istream& operator>>(istream& input, Cofetarie& c) //inline
		//friend => automat nu este metoda!
		//transmitere prin referinta pentru a pastra schimbarile!!
	{
		cout << "Nume: ";
		input >> c.nume;
		cout << "Nr angajati: ";
	...
	float& operator[](int index)
	{
		...
		{
			//in metode nu se afiseaza la consola
			//throw
			//try
			//catch
...
{
	//afisare toate campurile din Cofetarie

	output << "Nume:" << c.getNume() << endl;
	output << "Nr angajati:" << c.getNrAngajati() << endl;
